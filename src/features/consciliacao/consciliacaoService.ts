...
import { Conciliacao, CsvSisPag, NotaFiscal } from './types';
import  {validarCsv as validarCsvType, importarCsv as csvImport}  from './types';
...
export async function getNota(numNf: number): Promise<NotaFiscal> {
  try {
    const response = await api.get<NotaFiscal>( 
      `/Conciliacao/BuscarNota?numNf=${numNf}`
    );
    return response.data;
  } catch (error: unknown) {
    if (axios.isAxiosError<ApiResponse>(error)) {
